...
def train_and_test_on_umap_randcv(
    ...
    for _ in range(1):  # 100 iterations for RandomizedSearchCV
        ...
        for train_index, test_index in custom_folds:
            ...
            fig = plt.figure()
            ax = fig.add_subplot(111, projection='3d')
            sc = ax.scatter(X_test_reduced_shuffled[:, 0], X_test_reduced_shuffled[:, 1], X_test_reduced_shuffled[:, 2], c=actual_angle,
                            cmap='magma')
            ax.set_xlabel('UMAP 1')
            ax.set_ylabel('UMAP 2')
            ax.set_zlabel('UMAP 3')
            cbar = plt.colorbar(sc, ax=ax)
            ax.set_title('UMAP SHUFFLED test embeddings color-coded by head angle \n (allocentric) for fold: ' + str(
            ...
            # Set plot title
            fig.update_layout(title_text='UMAP test embeddings color-coded by angle (allocentric) for fold: ' + str(
                count) + ' rat id: ' + str(rat_id))
            ...
            # Set plot title
            fig.update_layout(title_text='UMAP test embeddings color-coded by angle (allocentric) SHUFFLED for fold: ' + str(
                count) + ' rat id: ' + str(rat_id))

            # Save the figure as an HTML file
            fig.write_html(f'{savedir}/umap_embeddings_fold_interactive_SHUFFLED_' + str(count) + '.html')
...
def main():
    ...
    big_df = pd.DataFrame()
    var_regress = 'anglesincos_allocentric'
