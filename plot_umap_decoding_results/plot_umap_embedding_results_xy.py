...
def train_and_test_on_umap_randcv(
    ...
    for _ in range(1):  # 100 iterations for RandomizedSearchCV
        ...
        for train_index, test_index in custom_folds:
            ...
            ax.set_title('UMAP test embeddings for fold (xy position): ' + str(count))
            plt.savefig(f'{savedir}/umap_embeddings_fold_' + str(count) + '.png', dpi = 300, bbox_inches = 'tight')
            fig = plt.figure()
            ax = fig.add_subplot(111, projection='3d')
            ax.scatter(X_test_reduced_shuffled[:, 0], X_test_reduced_shuffled[:, 1], X_test_reduced_shuffled[:, 2], c=color_data)
            #add x and y labels
            ax.set_xlabel('UMAP 1')
            ax.set_ylabel('UMAP 2')
            ax.set_zlabel('UMAP 3')
            ax.set_title('UMAP test shuffled embeddings for fold (xy position): ' + str(count))
            plt.savefig(f'{savedir}/umap_embeddings_fold_SHUFFLED_' + str(count) + '.png', dpi = 300, bbox_inches = 'tight')
            plt.close('all')
            ...
            ax.set_title('UMAP 1 vs UMAP 1 SHUFFLED for fold, (xy position): ' + str(count))
            ...
            # Set plot title
            fig.update_layout(
                title_text='UMAP test embeddings color-coded by xy position for fold (shuffled): ' + str(
            ...
            fig, ax = plt.subplots(1, 1)
            ax.imshow(colormap)
            ax.set_title('Color map for x and y position, Z-SCORED')
            #impose the x tick and y ticks
            ax.set_xticks([0, 255])
            ax.set_xticklabels([y_test[:, 0].min(), y_test[:, 0].max()])
            ax.set_yticks([0, 255])
            ax.set_yticklabels([y_test[:, 1].min(), y_test[:, 1].max()])
