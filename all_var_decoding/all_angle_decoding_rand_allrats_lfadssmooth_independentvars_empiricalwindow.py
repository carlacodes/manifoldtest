...
def main():
    ...
    for data_dir in [f'{base_dir}/rat_7/6-12-2019', f'{base_dir}/rat_10/23-11-2021',
        ...
        labels = np.load(
            f'{dlc_dir}/labels_250_scale_to_angle_range_True.npy')
        ...
        window_df = pd.read_csv(f'/ceph/scratch/carlag/honeycomb_neural_data/mean_p_value_vs_window_size_across_rats_grid.csv')
        #find the rat_id
        rat_id = data_dir.split('/')[-2]
        num_windows = window_df[window_df['rat_id'] == rat_id]['max_minimum_number_windows'].values[0]
        ...
        save_dir_path = Path(f'{data_dir}/randsearch_allvars_lfadssmooth_empiricalwindows_1000iter_independentvar_{now_day}')
