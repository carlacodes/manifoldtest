...
def main():
    ...
    labels = np.load(f'{dlc_dir}/labels_1203_with_goal_centric_angle_scale_data_False_zscore_data_False_overlap_False_window_size_50.npy')
    labels = np.load(f'{dlc_dir}/labels_50_raw.npy')
    col_list = np.load(f'{dlc_dir}/col_names_50_raw.npy')

    spike_data = np.load(f'{spike_dir}/inputs_10052024_50.npy')
    ...
    filename = f'params_all_trials_randomizedsearchcv_50bin_1000windows_jake_fold_{now}.npy'
    filename_mean_score = f'mean_score_all_trials_randomizedsearchcv_50bin_1000windows_jake_fold_{now_day}.npy'
    save_dir_path = Path(f'{data_dir}/randsearch_sanitycheck_allvars_parallel_array_50msbin_{now_day}')
    save_dir = save_dir_path
    save_dir.mkdir(exist_ok=True)

    
    window_df = pd.read_csv(f'/ceph/scratch/carlag/honeycomb_neural_data/mean_p_value_vs_window_size_across_rats_grid_50_windows_scale_to_angle_range_False_allo_True.csv')
        #find the rat_id
    rat_id = data_dir.split('/')[-2]
    #filter for window_size
    window_df = window_df[window_df['window_size'] == 50]
