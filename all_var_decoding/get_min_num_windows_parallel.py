...
from concurrent.futures import ProcessPoolExecutor
from helpers.tools import apply_lfads_smoothing
...
def process_rat_data(data_dir, scale_to_angle_range, big_df_savedir):
    df_across_windows = pd.DataFrame()
    for window_size in [100, 250]:
        rat_id = data_dir.split('/')[-2]
        spike_dir = os.path.join(data_dir, 'physiology_data')
        dlc_dir = os.path.join(data_dir, 'positional_data')
        ...
        X_for_umap, removed_indices = apply_lfads_smoothing(spike_data_copy)
        ...
        for h, val in enumerate(diff):
            if val == 1 and index_saved is None:
                index_saved = h
            if val != 1 and index_saved is None:
                index_saved = None
...
def run_ks_test_on_distributions_3d_grid(big_df_savedir, scale_to_angle_range=False):
    base_dir = '/ceph/scratch/carlag/honeycomb_neural_data/'
    data_dirs = [f'{base_dir}/rat_7/6-12-2019', f'{base_dir}/rat_10/23-11-2021',
                     f'{base_dir}/rat_8/15-10-2019', f'{base_dir}/rat_9/10-12-2021',
                     f'{base_dir}/rat_3/25-3-2019']

    with ProcessPoolExecutor() as executor:
        futures = [executor.submit(process_rat_data, dir, scale_to_angle_range, big_df_savedir) for dir in data_dirs]
        results = [f.result() for f in futures]

    df_across_windows = pd.concat(results)
    ...
    df_across_windows['max_minimum_number_windows'] = df_across_windows.groupby('rat_id')['minimum_number_windows'].transform('max')
    df_across_windows['max_minimum_number_windows_by_windowsize'] = df_across_windows.groupby('window_size')['minimum_number_windows'].transform('max')
    df_across_windows['max_minimum_number_windows_across_rats_and_windowsize'] = df_across_windows['max_minimum_number_windows'].max()
...
def main():
    ...
    #     'angle_rel_to_goal')
    # big_df = pd.DataFrame()
    big_df_savedir = '/ceph/scratch/carlag/honeycomb_neural_data/'
    # run_stratified_kfold_test()
    # run_ks_test_on_distributions(data_dir, param_dict, score_dict, big_df_savedir)
    run_ks_test_on_distributions_3d_grid(big_df_savedir, scale_to_angle_range=False)
    run_ks_test_on_distributions_3d_grid(big_df_savedir, scale_to_angle_range=True)
