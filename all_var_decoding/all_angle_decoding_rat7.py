...
def main():
    ...
    handler = logging.FileHandler(save_dir_path / f'bayes_search_{now}.log')
    ...
    if np.array_equal(spike_data, old_spike_data):
        logger.info('The spike data is the same as the old spike data')
    else:
        logger.info('The spike data is not the same as the old spike data')
