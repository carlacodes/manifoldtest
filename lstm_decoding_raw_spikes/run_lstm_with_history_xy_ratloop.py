...
def run_lstm_with_history(data_dir, rat_id = 'unknown_rat'):
    ...
    for i in range(0, X_for_lstm.shape[2]):
        ...
        score_df_neuron['neurosn_index'] = i
    ...
    big_score_df.to_csv(f'{csv_dir}/lstm_scores_{target_label}_rat_{rat_id}.csv')
    print('done')
...
def main():
    ...
    for rat in [3, 8, 9, 10, 7]:
        ...
        if Path(f'{csv_dir}/lstm_scores_xy_rat_{rat}.csv').is_file():
