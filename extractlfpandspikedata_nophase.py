...
def run_granger_cauality_test(df_theta_and_angle, export_to_csv = True, shuffle_data = False, direction = 'theta_before', rat = 'rat_3'):
    ...
    for trial in df_theta_and_angle['trial_number'].unique():
        ...
        plt.plot(df_trial['theta_phase'], label = 'Theta phase')
        plt.ylabel('Phase or angle')
        plt.xlabel('Time since start of trial (s)')
        plt.xticks(np.arange(0, len(df_trial['dlc_angle_phase']), 1000*50), labels=np.arange(0, len(df_trial['dlc_angle_phase'])/1000, 50))
        plt.legend()
        plt.title(f'DLC angle and theta phase for trial number {trial}, shuffled = {shuffle_data}', fontsize = 20)
        plt.savefig(f'/ceph/scratch/carlag/honeycomb_neural_data/{rat}/dlc_angle_theta_phase_trial_{trial}_shuffle_{shuffle_data}_direction_{direction}_{rat}_nophaseheadangle.png', dpi=300, bbox_inches='tight')
    ...
    try:
        ...
        return
    if export_to_csv:
        granger_dataframe_all_trial.to_csv(f'//ceph/scratch/carlag/honeycomb_neural_data/csvs/granger_trial_cumulative_shuffle_{shuffle_data}_direction_{direction}_{rat}_regheadangle.csv')
...
def compare_simulated_data_to_granger_test(n_samples):
    ...
    df_result_xy.to_csv('/nfs/nhome/live/carlag/granger_lfp/manifoldtest/csvs/granger_simulated_xy_CORRELATED.csv')
    df_result_xz.to_csv('/nfs/nhome/live/carlag/granger_lfp/manifoldtest/csvs/granger_simulated_xz_UNCORRELATED.csv')
...
def main():
    ...
    for rat in [ 'rat_7', 'rat_3', 'rat_8', 'rat_9', 'rat_10']:  #rat_3
        print('Running granger causality test for rat: ' + rat)
        path_to_load = Path('/ceph/scratch/carlag/honeycomb_neural_data') / rat
        ...
        granger_results = run_granger_cauality_test(df_theta_and_angle, shuffle_data=False, direction='theta_before', rat=rat)
        granger_results = run_granger_cauality_test(df_theta_and_angle, shuffle_data=True, direction='theta_before', rat=rat)
