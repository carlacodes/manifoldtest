...
def cat_dlc(windowed_dlc, scale_to_angle_range=False):
    ...
    for i, k in enumerate(windowed_dlc):
        if i == 0:
            column_names = ['trial', 'x', 'y', 'dist2goal', 'hd', 'relative_direction_to_goal']
            total_num_cols = 8  # trial, x, y, dist2goal, sin(hd), cos(hd), sin(relative_direction), cos(relative_direction)
        ...
        temp_array[:, count] = i  # Add trial number
        col_names_actual[count] = 'trial'
        count += 1
