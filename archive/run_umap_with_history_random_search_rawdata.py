...
def main():
    ...
    #load labels
    labels = np.load(f'{dlc_dir}/labels_2902_with_dist2goal_scale_data_False_zscore_data_True.npy')
    spike_data = np.load(f'{spike_dir}/inputs.npy')
    #find the times where the head angle is stationary
    angle_labels = labels[:, 5]
    ...
    for params in ParameterSampler(param_grid_upper, n_iter=n_iter):
        ...
        regress = 'dlc_angle_raw'  # Assuming 'head_angle' is the column in your DataFrame for regression
        ...
        filename = f'params_reducrandom_raw_{now}.npy'
        filename_intermediate_params = f'intermediate_params_reducrandom_random_search_raw_{now_day}.npy'
