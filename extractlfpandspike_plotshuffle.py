...
def run_granger_cauality_test(df_theta_and_angle, export_to_csv = True, shuffle_data = False, direction = 'theta_before', rat = 'rat_3', no_phase = False):
    ...
    for trial in df_theta_and_angle['trial_number'].unique():
        ...
        for count, key in enumerate(granger_test.keys()):
            ...
            corrected_bonferroni_p = granger_test_for_indiv_lag[1] * int(key)
    ...
    try:
        ...
        return
    if export_to_csv:
        granger_dataframe_all_trial.to_csv(f'csvs_2/granger_trial_cumulative_shuffle_{shuffle_data}_direction_{direction}_{rat}_phase_{no_phase}.csv')
    ...
    for trial in df_theta_and_angle['trial_number'].unique():
        ...
        if not is_stationary_angle or not is_stationary_theta:
            ...
            if shuffle_data == True:
                ...
                plt.savefig(
                    f'//ceph/scratch/carlag/honeycomb_neural_data/figures/dlc_angle_theta_phase_trial_{trial}_noshuffle_direction_{direction}_{rat}_nophase_{no_phase}.png',
        ...
        if no_phase == True:
            ...
            plt.legend()
            plt.title(f'DLC angle and theta phase for trial number {trial}, shuffled = {shuffle_data}', fontsize = 20)
            plt.savefig(f'/ceph/scratch/carlag/honeycomb_neural_data/figures/trial_{trial}_shuffle_{shuffle_data}_direction_{direction}_{rat}_nophase_{no_phase}.png', dpi=300, bbox_inches='tight')
        else:
            plt.figure(figsize=(40, 10))
            plt.plot(theta_phase_trial, label = 'Theta phase', color = 'orange', alpha = 0.2)
            ...
            plt.legend()
            plt.title(f'DLC angle and theta phase for trial number {trial}, shuffled = {shuffle_data}', fontsize = 20)
            plt.savefig(f'/ceph/scratch/carlag/honeycomb_neural_data/figures/dlc_angle_theta_phase_trial_{trial}_shuffle_{shuffle_data}_direction_{direction}_{rat}_nophase_{no_phase}.png', dpi=300, bbox_inches='tight')
    return
