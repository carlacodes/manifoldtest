...
spks originally was  a numpy arrray of size trial* timebins*neuron, and bhv is  a pandas dataframe where each row represents a trial, the trial is the index '''
from sklearn.decomposition import PCA
...
def unsupervised_pca(spks, bhv):
    ...
    plt.savefig('/ceph/scratch/carlag/honeycomb_neural_data/rat_7/6-12-2019/figures/latent_projections/pca_angle.png', bbox_inches='tight')
    plt.show()
    ...
    ax.set_zlabel('PCA3')
    plt.title('PCA projection of the dataset', fontsize=24)
    plt.savefig('/ceph/scratch/carlag/honeycomb_neural_data/rat_7/6-12-2019/figures/latent_projections/pca_angle_3d.png', bbox_inches='tight')
    plt.show()
...
def unsupervised_umap(spks, bhv, remove_low_variance_neurons = True, neuron_type = 'unknown', filter_neurons = False, n_components = 3):
    ...
    if n_components == 2:
        ...
        for var in list_of_vars:
            ...
            if filter_neurons:
                plt.savefig(f'/ceph/scratch/carlag/honeycomb_neural_data/rat_7/6-12-2019/figures/latent_projections/umap_angle_3d_colored_by_{var}_neuron_type_{neuron_type}.png',
                            bbox_inches='tight', dpi=300)
            else:
                plt.savefig(f'/ceph/scratch/carlag/honeycomb_neural_data/rat_7/6-12-2019/figures/latent_projections/umap_angle_3d_colored_by_{var}_num_components_{n_components}_all_neurons.png',
                            bbox_inches='tight', dpi=300)
            plt.show()
        ...
        for var in list_of_vars:
            ...
            plt.title(f'UMAP projection of the dataset, color-coded by: {var}', fontsize=15)
            if filter_neurons:
                plt.savefig(f'/ceph/scratch/carlag/honeycomb_neural_data/rat_7/6-12-2019/figures/umap_angle_3d_colored_by_{var}_neuron_type_{neuron_type}.png', bbox_inches='tight', dpi=300)
            else:
                plt.savefig(f'/ceph/scratch/carlag/honeycomb_neural_data/rat_7/6-12-2019/figures/umap_angle_3d_colored_by_{var}_all_neurons_num_components_{n_components}.png', bbox_inches='tight', dpi=300)
            plt.show()
...
def train_ref_classify_rest(
    ...
    results_cv = Parallel(n_jobs=n_jobs_para, verbose=1, prefer="threads")(
...
def main():
    ...
    for run in range(n_runs):
        ...
        print(f'results for this iteration of {run}:')
        print(results_between[run])
        

        # Save results
    results = {'between': results_between, 'within': results_within}
    save_path = Path('/ceph/scratch/carlag/honeycomb_neural_data/rat_7/6-12-2019/')
